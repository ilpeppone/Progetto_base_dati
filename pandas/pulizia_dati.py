...
df_artisti['name'] = df_artisti['name'].astype(pd.StringDtype())
df_artisti['gender'] = df_artisti['gender'].astype(pd.StringDtype())
...
df_artisti['placeOfBirth'] = df_artisti['placeOfBirth'].astype(pd.StringDtype())
df_artisti['placeOfDeath'] = df_artisti['placeOfDeath'].astype(pd.StringDtype())
...
df_lavori['accession_number'] = df_lavori['accession_number'].astype(pd.StringDtype())
df_lavori= df_lavori.drop(columns=['artist'])#artist lo tolgo perchè il nome è già presente nel csv degli artisti
df_lavori['artistRole'] = df_lavori['artistRole'].astype(pd.StringDtype())
df_lavori['artistId'] = pd.to_numeric(df_lavori['artistId'],  errors='coerce').astype(pd.Int32Dtype())
df_lavori['title'] = df_lavori['title'].astype(pd.StringDtype())
df_lavori['dateText'] = df_lavori['dateText'].astype(pd.StringDtype())#lo lascio perchè ci sono informazioni aggiuntive
df_lavori['medium'] = df_lavori['medium'].astype(pd.StringDtype())
df_lavori['creditLine'] = df_lavori['creditLine'].astype(pd.StringDtype())
...
df_lavori['dimensions'] = df_lavori['dimensions'].astype(pd.StringDtype())
...
df_lavori['inscription'] = df_lavori['inscription'].astype(pd.StringDtype())
df_lavori['thumbnailCopyright'] = df_lavori['thumbnailCopyright'].astype(pd.StringDtype())
df_lavori['thumbnailUrl'] = df_lavori['thumbnailUrl'].astype(pd.StringDtype())
df_lavori['url'] = df_lavori['url'].astype(pd.StringDtype())
